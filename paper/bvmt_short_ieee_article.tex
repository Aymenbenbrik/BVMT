...
\begin{abstract}
We present a reproducibility study of multi-component deep forecasting on a thin emerging market (BVMT, 68 stocks, 2016--2025, 116{,}626 records). The pipeline combines a Temporal Fusion Transformer (TFT), XGBoost fundamentals, French NLP sentiment, a Graph Attention Network, and SHAP. On a harmonized 2025 test slice ($N=5{,}582$, UP ratio $49.84\%$), the Vertex-reproducible TFT v3 reaches $51.79\%$ directional accuracy (95\% CI $[50.43, 53.08]$), statistically indistinguishable from ten classical and deep baselines and from always-UP (pairwise McNemar $p \geq 0.35$). Two internal TFT ablations move accuracy by $\leq 0.4$~pp ($p > 0.65$); the per-stock target normalizer and the Variable Selection Network are not measurably helpful, and empirical $80\%$ coverage of the central interval is $78.25\%$ ($-1.75$~pp from nominal). An earlier internal estimate of $77.4\%$ is documented as non-reproducible from released artefacts and retained transparently as a development-time number. A multi-agent ablation on offline-realizable configurations (TFT alone, TFT$+$Graph via a Pearson-correlation proxy, $\pm$ gating) shows the graph signal cannot flip the binary technical decision under deployed weights; seven configurations remain DB-dependent. The contributions are (i) a reproducible Vertex-AI pipeline with two TFT ablations, (ii) ten baselines plus three TFT variants harmonized on one slice with bootstrap CIs and pairwise McNemar tests, (iii) a deflated-Sharpe backtest exposing a long-only versus long-short ranking inversion ($\rho=-0.27$) on the $+28\%$-drift 2025 test year, and (iv) a documented gap between an earlier headline and the reproducible numbers, offered as a reproducibility lesson for thin-market AI research.
\end{abstract}
...
\textbf{Multi-agent systems and our positioning.} Classical autonomous Multi-Agent Systems~\cite{ref:multi_agent} feature heterogeneous learning agents trading on a shared environment; recent LLM-based frameworks~\cite{ref:yang_fingpt,ref:lopez_lira_chatgpt} use large language models as analyst/trader/risk-officer personas. Our system belongs to neither tradition: the seven components are specialized deterministic Python modules invoked by a central orchestrator, with typed structured outputs, no peer-to-peer communication, and no shared mutable state. The orchestrator computes a data-quality-adjusted linear combination of typed component outputs; formally the system is a \emph{weighted ensemble with typed interfaces, deterministic components, and centralized failure isolation}~\cite{ref:ensemble}. We retain the \emph{multi-component} vocabulary in section titles and code identifiers (e.g., \texttt{AgentSignal}) for continuity with the released codebase, while explicitly distancing the architecture from the autonomous-MAS and LLM-agent traditions cited above.
...
\textbf{Quantile calibration.} The pinball quantile loss minimises a proper scoring rule but does not by itself guarantee that the predicted intervals are empirically well-calibrated~\cite{ref:kuleshov,ref:confidence_calibration}. The deployed v3 predicts $Q_{0.1}$, $Q_{0.5}$, $Q_{0.9}$ of the seven-day fractional-return distribution; if calibrated, the realised return should lie in $[\hat{Q}_{0.1}, \hat{Q}_{0.9}]$ on $80\%$ of rows. On the de-duplicated harmonized slice ($N=5{,}582$), empirical coverage is $\mathbf{78.25\%}$ ($4{,}368/5{,}582$), a gap of $-1.75$~pp from the nominal target --- consistent with mild overconfidence of the central interval; on the raw multi-horizon grid ($N=6{,}483$ after dropping rows with missing quantiles) it is $78.50\%$. Because confidence in Section~\ref{sec:methodology}.C reads from the central interval width, this slight under-coverage propagates to a small overconfidence in the gating decision. The per-row CSV ships all three quantiles, enabling the calculation to be replicated and the gating threshold to be re-tuned without re-training.

\subsection{Multi-Agent Ablation (Offline-Realisable Subset)}\label{sec:multiagent_ablation}

Of the nine planned configurations, three are realisable from offline TFT v3 predictions plus raw price history (A1, A4-proxy, A8-proxy, where the \texttt{-proxy} suffix denotes substitution of a structural Pearson correlation for the trained GAT of A4); the trained-GAT variant (A4-GAT) and the six remaining configurations (A2, A3, A5, A6, A7, A9) require the deployed PostgreSQL database (\texttt{news\_articles} pre-scores + \texttt{financial\_ratios}) and the trained model checkpoints (XGBoost fundamental, GAT graph), none of which is shipped as offline data. To still test the graph contribution offline we replace the trained GAT by a \emph{structural proxy} (suffix \texttt{-proxy} throughout): $60$-day Pearson correlation across all $68$ tickers, top-$10$ neighbours with $|\rho| \geq 0.30$, graph score $=$ signed-correlation-weighted neighbour TFT v3 predictions in $[-1, +1]$, confidence $= \overline{|\rho|}$ of selected neighbours. The proxy is intended as a lower bound on the trained GAT's directional skill: it preserves the contemporaneous correlation structure and the neighbour-aggregation rule, but discards the learned attention weights and lead-lag information that a real GAT could in principle exploit. Coverage is $69\%$ of rows ($3{,}851/5{,}582$), of which $61.5\%$ ($3{,}435$) clear the deployed orchestrator gate ($\text{conf}\!\ge\!0.20$ and $|\text{score}|\!\ge\!0.08$).

Table~\ref{tab:multiagent_ablation} summarises the result. A1 $=$ A4-proxy $=$ A8-proxy $=$ $51.79\%$ (zero flips, McNemar $p = 1.000$). With deployed weights ($w_\text{tech}\!=\!0.34$, $w_\text{graph}\!=\!0.20$, orchestrator thresholds $0.55/0.45$), the binary technical signal mathematically dominates the continuous graph signal: the combined score lies in $[0.63, 1.0]$ when technical$=$UP and in $[0, 0.37]$ when technical$=$DOWN, so the graph can never flip the decision. A weight-sensitivity sweep (Table~\ref{tab:weight_sensitivity}) confirms the graph begins to flip predictions only at $w_\text{graph} \geq 0.50$ and never significantly differs from A1. The offline graph proxy carries no detectable directional skill above TFT v3 alone, regardless of weighting strategy; whether the trained GAT (A4-GAT) lifts this conclusion is left for the database-snapshot replay (Section~\ref{sec:conclusion}.B).
...
A4-proxy  & + Graph (Pearson proxy, deployed weights)   & $51.79\%$ & $\pm 0.00$ pp   & $1.000$   \\
A8-proxy  & A4-proxy $-$ gating (graph always used)     & $51.79\%$ & $\pm 0.00$ pp   & $1.000$   \\
\midrule
A4-GAT    & + Graph (trained GAT)                       & \multicolumn{3}{l}{\textit{DB-dependent}} \\
...
\midrule
\multicolumn{5}{l}{\textit{Reference baselines from Table~\ref{tab:baselines} (same slice, $N=5{,}582$):}} \\
\multicolumn{2}{l}{Logistic regression (L2)}              & $52.79\%$ & $+1.00$ pp      & $0.351$   \\
\multicolumn{2}{l}{Always-UP (slice majority)}            & $49.84\%$ & $-1.95$ pp      & $0.054$   \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Weight-sensitivity sweep for A4-proxy: varying the orchestrator's graph weight $w_\text{graph}$ with $w_\text{tech} = 1 - w_\text{graph}$, gate active. ``Flips'' counts per-row predictions where A4-proxy disagrees with A1.}
...
\textbf{Falsifiable hypotheses, status on the offline subset.} (i) If $|\,\text{A6} - \text{A1}\,|\!<\!1$ pp the ensemble layer is performance-neutral; for the offline-realisable subset \{A1, A4-proxy, A8-proxy\} the gap is $0.00$ pp, consistent with the ensemble contributing via robustness and auditability rather than accuracy. (ii) A8-proxy $=$ A4-proxy exactly (no flips), so confidence gating contributes nothing to accuracy in this 2-agent ensemble; the gating-defense argument of Section~\ref{sec:security} survives as an information-theoretic property (cannot be worse than A4-proxy) but is empirically inactive on this dataset. (iii) Per-segment breakdown (banks $\mid$ insurance $\mid$ non-banks) and the trained-GAT contribution (A4-GAT vs.\ A4-proxy) remain DB-dependent.

\subsection{Economic Backtest of Baselines}\label{sec:economic}

Directional accuracy above the random floor does not guarantee exploitability: spread, volume, and execution friction can wipe the signal out, and on a thin market with a structural directional drift even a high accuracy can lose to buy-and-hold. The backtest-overfitting literature in quantitative finance has long documented how multiple-testing inflation and out-of-sample over-fitting produce strategies that look statistically significant in development and disappear in deployment~\cite{ref:bailey_pseudo,ref:harvey_presidential}. We reconstruct a pseudo-PnL series for every baseline by converting each row's prediction into a $7$-day position opened at \texttt{close}$_t$ and closed at \texttt{close}$_{t+7}$, equal-weighted across active tickers, charging a $50$ bps round-trip cost per position (conservative end of BVMT bid-ask + brokerage). We report the deflated Sharpe ratio of~\cite{ref:deflated_sharpe} with $k\!=\!9$ trials. Table~\ref{tab:economic} reports the long-only NET regime (always-DOWN is flat under long-only and is omitted). Two caveats: (a) the backtest evaluates the broader 2025 test set ($N=10{,}013$ rows after boundary drop), wider than the directional-accuracy slice ($N=5{,}582$) because reconstructed $7$-day forward returns are independent of the de-duplication used in Section~\ref{sec:directional_acc}; (b) consecutive rows share overlapping forward-return windows, inducing serial correlation that inflates the raw Sharpe relative to a non-overlapping convention. A Newey-West adjustment with lag $6$ would widen the standard errors of the Sharpe values reported below; the ordering of strategies is preserved but absolute statistical significance should be interpreted accordingly.
...
Three observations: (i) the 2025 test year had a strong positive drift (always-UP earns $+28\%$ cumulative net) so the binding benchmark is always-UP, not always-DOWN; (ii) four baselines clear the deflated-Sharpe bar in long-only (Standard Transformer, LSTM, LogReg L2, Momentum 5/20), but under a symmetric long-short rule (positions flip to $-1$ on pred$=$DOWN) the ranking inverts: every model except always-UP loses heavily (Sharpe $-5$ to $-18$, cumulative $-42$ to $-93\%$), zero of nine clear DSR $p<0.05$, and the Spearman rank correlation between the long-only and long-short Sharpe orderings is $\rho=-0.27$. The Standard Transformer's long-only Sharpe of $4.40$ therefore reflects selective long-only exposure on a $+28\%$-drift year rather than genuine symmetric directional skill --- under long-short the same predictions destroy $75\%$ of capital. (iii) Within the long-only regime the deep baselines concentrate their directional signals on days where realised returns are large, producing better Sharpe and drawdown than logistic regression despite indistinguishable pooled accuracy --- a behaviour that raw accuracy misses. TFT v3's economic profile on the harmonized slice is shipped at \path{results/baselines/vertex_slice_harmonized_predictions.csv} and falls in the LSTM/Transformer range, consistent with the directional-accuracy ranking.
...
This section is \emph{architectural}: it describes threat models and audit-trail properties that follow from the design choices. An empirical adversarial study --- including data-poisoning~\cite{ref:data_poisoning} and a SHAP-fooling baseline~\cite{ref:slack_fooling} --- is left to future work.

\textbf{Thin-market poisoning surface.} An adversary with modest capital can manipulate a single illiquid stock to shift global statistics. Per-stock target normalization isolates each $(\mu_i, \sigma_i)$ from manipulation of stock $j$, and per-encoder-window feature scaling resets local statistics every $L=30$ days. The attack surface reduces to direct signal injection inside a single window of a single stock; confidence gating ($w_\text{gated} = w_\text{orig} \cdot \text{conf}/\tau$) provides a secondary defense by down-weighting low-confidence outputs. The TFT internal ablations of Section~\ref{sec:tft_ablations} show these defenses impose no measurable accuracy cost on BVMT 2025.

\textbf{XAI as audit trail.} Each prediction emits a human-readable explanation containing per-agent contributions, feature attributions (SHAP for XGBoost, attention weights for TFT), and confidence breakdowns --- the technical building blocks for the transparency obligations of EU AI Act Articles~13--14~\cite{ref:eu_ai_act}. Full conformity additionally depends on governance, data governance, technical documentation, post-market monitoring, and where applicable conformity assessment by a notified body (Arts.~9, 10, 43, 72; Annex IV), none of which this paper claims. SHAP faithfulness on BVMT data and adversarial robustness of explanations~\cite{ref:alvarez_melis,ref:kumar_shap} remain unvalidated; we therefore phrase every regulatory claim as \emph{aligned with} or \emph{designed to support} the Act, not as ``satisfying'' it.

\section{Reproducibility and Artifact Availability}\label{sec:repro}

The released artefacts support a one-command reproduction of every numerical claim in this paper. (i) \textbf{Training pipeline}: the Vertex-AI container (\path{vertex/Dockerfile}, \path{vertex/cloudbuild.yaml}) and the ablation runner (\path{training/tft_ablations/runner.py}) re-train the TFT v3 reference and the two internal ablations (TFT $-$ VSN, TFT $-$ per-stock norm) bit-for-bit from \texttt{BVMT\_SEED}, in $71$--$161$ minutes on a single NVIDIA T4. (ii) \textbf{Per-row predictions}: \path{results/tft_ablations/vertex/baseline_test_predictions.csv} contains $(Q_{0.1}, Q_{0.5}, Q_{0.9}, r_7^\text{true})$ for every test row of the v3 reference and its two ablations. (iii) \textbf{Harmonized panel}: \path{results/baselines/vertex_slice_harmonized_predictions.csv} contains per-row predictions for all ten non-TFT baselines plus the three TFT variants on the canonical $N=5{,}582$ slice; the matching \texttt{*.json} carries every number in Table~\ref{tab:baselines}. (iv) \textbf{Multi-agent sweep}: \path{scripts/multiagent_offline_sweep.py} reproduces Tables~\ref{tab:multiagent_ablation} and~\ref{tab:weight_sensitivity} in $\sim\!60$~s on commodity CPU. (v) \textbf{Sentiment validation tooling}: \path{scripts/sample_news_for_labelling.py} and \path{scripts/evaluate_sentiment_labels.py} produce the 200-article expert-labelling sample and the Cohen's $\kappa$ evaluation report listed in the limitations.
...
We present a reproducibility-first multi-component decision pipeline for thin-market equity forecasting on BVMT 2016--2025. The Vertex-reproducible TFT v3 reference reaches $51.79\%$ directional accuracy (95\% CI $[50.43, 53.08]$) on the harmonized 2025 test slice, statistically indistinguishable from ten classical and deep baselines and from always-UP. Two internal ablations move accuracy by $\leq 0.4$~pp ($p > 0.65$), providing no significant evidence for or against the original architectural hypotheses. Empirical $80\%$ coverage of the predicted central interval is $78.25\%$ on the slice ($-1.75$~pp from nominal), indicating mild overconfidence. A multi-agent offline-realizable subset (A1, A4-proxy, A8-proxy) shows the graph signal cannot flip the binary technical decision under the deployed orchestrator weights; the trained-GAT variant A4-GAT and six others (A2, A3, A5, A6, A7, A9) are DB-dependent. An economic backtest extends the analysis: the long-only and long-short deflated-Sharpe rankings are nearly uncorrelated (Spearman $\rho=-0.27$; $0/9$ baselines clear DSR $p<0.05$ in long-short despite four doing so in long-only), confirming that the apparent edge of deep baselines reflects selective exposure on a $+28\%$-drift year rather than symmetric directional skill. An earlier internal estimate of $77.4\%$ is documented as non-reproducible and retained as a transparency disclosure.
...
\item \textbf{Multi-agent ablation coverage}: only 3 of 9 configurations realisable offline (A1, A4-proxy, A8-proxy); the trained-GAT variant (A4-GAT) and six others (A2, A3, A5, A6, A7, A9) require the deployed PostgreSQL database (news pre-scores, financial ratios) and the trained model checkpoints (XGBoost fundamental, GAT graph).
...
(i) Replay A2, A3, A4-GAT, A5, A6, A7, A9 once the database snapshot is published. (ii) Empirical poisoning study validating the architectural defenses of Section~\ref{sec:security}. (iii) Re-calibration of the central interval to close the $1.75$~pp gap to $80\%$ nominal coverage, and re-tuning of the confidence-gating thresholds against the calibrated widths. (iv) AraBERT-based Arabic sentiment integration to lift coverage from $85\%$ to $100\%$. (v) Per-stock breakdown table with bootstrap CI and lift over always-UP on the harmonized slice (CSV shipped, table omitted for space).
