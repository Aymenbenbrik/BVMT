...
TFT v3 achieves 77.4\% directional accuracy on the unseen 2025 test set, compared to 53.8\% for CNN-LSTM best run. The 2025 test set contains $N_{\mathrm{test}} = 10{,}314$ row-level predictions over 241 trading days and 43 of the 68 tickers (the remaining 25 tickers had no actively trading days in 2025 in the cleaned dataset; this gap is acknowledged as a coverage limitation in Section~\ref{sec:conclusion}). The 23.6 percentage point absolute improvement over CNN-LSTM is consistent across all CNN-LSTM hyperparameter configurations evaluated; statistical significance versus the always-DOWN baseline (Table~\ref{tab:baselines}) and per-stock performance are reported in Phase~1 of the improvement plan once the competitive baselines are trained.

\section{Results and Discussion}\label{sec:results}

\subsection{Contextualizing Directional Accuracy}

The 77.4\% directional accuracy must be interpreted in the context of market class imbalance. Across all 68 stocks and all 10 years of data, the market exhibits a 41.9\% UP ratio --- only 41.9\% of 7-day windows show positive returns. The bearish bias makes \emph{always-DOWN}, not the uniform 50/50 random classifier, the relevant naive baseline:

\begin{table}[htbp]
\centering
\caption{TFT v3 directional accuracy versus the three relevant naive baselines on the 2025 test set.}
\label{tab:baselines}
\small
\begin{tabular}{lcc}
\toprule
\textbf{Baseline} & \textbf{Accuracy} & \textbf{TFT v3 lift} \\
\midrule
Always-UP                       & 41.9\% & $+35.5$ pp \\
Random uniform 50/50            & 50.0\% & $+27.4$ pp \\
\textbf{Always-DOWN (majority)} & \textbf{58.1\%} & \textbf{$+19.3$ pp} \\
TFT v3 (this work)              & 77.4\% & --- \\
\bottomrule
\end{tabular}
\normalsize
\end{table}

The honest comparison is therefore $+19.3$ percentage points over always-DOWN, not the often-quoted $+27.4$ pp over random. We retain the random reference for cross-paper comparability, but flag that always-DOWN is the only naive baseline a quantitative finance reviewer will accept on a thin market with structural bearish bias. Per-stock accuracy and statistical significance versus always-DOWN (McNemar test) are reported in Section~\ref{sec:experiments} once the competitive baselines from Phase~1 of the improvement plan are available.
...
Two version-agnostic measures of skill are appropriate here: the directional accuracy reported in Section~\ref{sec:results}.A and Table~\ref{tab:baselines} (an $+19.3$ pp lift of v3 over the always-DOWN baseline), and the empirical quantile-coverage analysis discussed in the next subsection. A like-for-like BCE comparison of v3 against v1 would require a binary-classification re-run of the v3 architecture and is identified as a deliverable of Phase~1 in our improvement plan.
