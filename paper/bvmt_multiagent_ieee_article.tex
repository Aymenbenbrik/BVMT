...
Most published TFT applications target developed markets (S\&P 500, NASDAQ, European indices) with reported directional accuracy ranges of 55--65\% on daily forecasts. Emerging markets, by contrast, exhibit time-varying integration with global markets~\cite{ref:bekaert_harvey} and well-documented thin-liquidity effects that complicate factor stability and out-of-sample reproducibility~\cite{ref:harvey_multiple,ref:lopez_causal}. This paper demonstrates that with careful adaptation to thin market conditions --- specifically per-stock target normalization, per-encoder-window feature scaling, and confidence gating --- TFT can achieve 77.4\% on emerging market data with an equivalent or longer prediction horizon (seven days).
