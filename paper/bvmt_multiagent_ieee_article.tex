...
\begin{table*}[htbp]
\centering
\caption{Directional accuracy of TFT v3 versus naive, trend-following, and tabular-ML baselines on the 2025 test set. Trivial / trend rows: $N=10{,}314$ row-level predictions, 43 tickers, $49.98\%$ UP, produced by \texttt{python -m training.baselines.trivial}. Tabular-ML rows: $N=10{,}013$ row-level predictions (windowed 60-day inputs lose the first $59$ rows of each test ticker), produced by \texttt{python -m training.baselines.ml\_classics}. All confidence intervals are 95\% percentile-bootstrap on the test set (1{,}000 resamples). Hyperparameters were selected on the 2024 validation split. The TFT~v3 row reproduces the headline figure from Section~\ref{sec:experiments}; per-row predictions are deferred to improvement-plan Action~1.8.}
\label{tab:baselines}
\small
...
\item \textbf{Tabular ML adds modest but real signal.} Logistic regression beats the trivial floor at $52.88\%$ (95\% CI $[51.98, 53.95]$), and McNemar against random forest gives $p = 4.1 \times 10^{-4}$, against XGBoost $p \approx 0.053$. Random forest with deep trees overfits the validation split (val $53.7\% \to$ test $50.6\%$, a $-3.0$ pp generalisation drop); XGBoost sits in between at $51.63\%$. None of the three exceeds $53\%$ on the test set despite a 900-dimensional flattened input that spans the same 60 days CNN-LSTM consumes.
\item \textbf{The TFT v3 lift over every classical-ML baseline is approximately $+24$ to $+27$ pp.} Even versus the best tabular ML model (LogReg, $52.88\%$), the gap is $77.4 - 52.88 = 24.5$ pp. This is a much larger gap than the supervisor critique anticipated and warrants Phase~1 internal-ablation scrutiny (Action~1.4: TFT $-$ VSN, $-$ per-window normalization, $-$ confidence gating). A gap of this magnitude is consistent either with a genuine architectural advantage (most plausibly the Variable Selection Network's per-feature gating, which has no analogue in a flattened $900$-d vector) or with a residual leak the present audit has not yet caught --- the Phase~1 ablations are designed to discriminate between the two.
...
Per-stock accuracy and statistical significance versus the strongest tabular baseline (McNemar test) are reported in Section~\ref{sec:experiments} once the TFT v3 per-row predictions become available. Across the eight baselines tested in Table~\ref{tab:baselines}, mean per-ticker accuracy stays in $[49.9\%, 53.5\%]$, with the per-ticker IQR roughly $\pm 5$ pp; none of the eight has a single-ticker accuracy exceeding $65\%$ on the 2025 test set.
...
\begin{table*}[htbp]
\centering
\caption{Economic-backtest metrics for the seven baselines on the 2025 test set, long-only with $50$ bps round-trip transaction cost. Annualised Sharpe uses $r_f = 8\%$ (BCT TMM proxy). The deflated-Sharpe $p$-value (DSR$_p$) corrects for the fact that seven strategies are tested simultaneously \cite{ref:deflated_sharpe}; bold $p$-values clear the conventional $0.05$ threshold. Always-DOWN is permanently flat under the long-only convention so its row is omitted.}
...
\midrule
Always-UP                              & $+27.95\%$ & $+32.12\%$ & $1.24$ & $-27.27\%$ & $1.31$ & $0.538$ \\
Momentum 5/20                          & $+47.75\%$ & $+53.05\%$ & $3.40$ & $-12.70\%$ & $1.95$ & $\mathbf{0.031}$ \\
MA$_{20}$ crossover                    & $+38.05\%$ & $+42.35\%$ & $2.54$ & $-15.48\%$ & $1.69$ & $0.131$ \\
\textbf{Logistic regression (L2)}      & $\mathbf{+45.82\%}$ & $\mathbf{+50.72\%}$ & $\mathbf{3.69}$ & $-12.55\%$ & $2.07$ & $\mathbf{0.017}$ \\
Random forest                          & $+4.24\%$  & $+4.66\%$  & $-0.75$ & $-10.07\%$ & $1.20$ & $0.977$ \\
XGBoost                                & $+20.39\%$ & $+22.47\%$ & $1.66$ & $-17.01\%$ & $1.51$ & $0.381$ \\
\bottomrule
\end{tabular}
\normalsize
\end{table*}
...
\begin{itemize}
\item \textbf{The 2025 test year had a strong positive drift}: a long-only always-UP rule earns $+27.95\%$ cumulative with Sharpe $1.24$ and a $-27\%$ drawdown when crowded out at the wrong entry days. The deflated-Sharpe $p$-value $0.538$ confirms this is not statistically distinguishable from luck once seven strategies are pooled. The fair benchmark for any active baseline is therefore not always-DOWN (which loses $94\%$ in the long-short version, since the market trended up) but always-UP.
\item \textbf{Two baselines clear the deflated-Sharpe bar at $p<0.05$.} Momentum~5/20 (Sharpe $3.40$, DSR$_p = 0.031$) and Logistic Regression L2 (Sharpe $3.69$, DSR$_p = 0.017$) both outperform always-UP after accounting for transaction costs and multiple testing. They are the only baselines whose $2025$ Sharpe is more likely real than a multiple-testing artefact under our setup.
\item \textbf{Heavier ML hurts here.} Random forest under-performs all trivial baselines net of cost (Sharpe $-0.75$, $+4.2\%$ cumulative on a $+28\%$ market). XGBoost trails always-UP by $7.5$ pp of cumulative return. The TFT v3 economic backtest is identified as a deliverable of Action~1.8 (per-row prediction CSV $\rightarrow$ \texttt{training.baselines.economic\_metrics}); it is necessary because directional accuracy alone, even at $77.4\%$, does not establish exploitability when round-trip costs and the market's positive drift are taken into account.
\end{itemize}
